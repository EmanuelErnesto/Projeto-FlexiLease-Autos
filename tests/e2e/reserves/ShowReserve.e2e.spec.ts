...
describe('DeleteReserveController', () => {
  ...
  it('Should be able to return a existent Reserve', async () => {
    const createUser = {
      name: 'Josué Motoboy',
      cpf: '123.456.789-00',
      birth: '10/05/2000',
    ...
    const response = await supertest(app)
      .get(reserveUrl + '/' + createdReserve.body._id)
      .set('Authorization', `Bearer ${token}`);

    expect(response.status).toEqual(HttpStatusCode.Ok);
    expect(response.body.start_date).toEqual(createReservation.start_date);
    expect(response.body.end_date).toEqual(createReservation.end_date);
  });

  it('Should not be able to return a reserve that does not exists', async () => {
    const createUser = {
      name: 'Josué Motoboy',
      cpf: '123.456.789-00',
      birth: '10/05/2000',
    ...
    const response = await supertest(app)
      .get(reserveUrl + '/' + '668af1e3b9ea0673bc61fd9e')
      .set('Authorization', `Bearer ${token}`);
